...
export default async function handler(req, res) {
  ...
  if (section === 'users_bulk') {
    ...
    await appendAdminWebAudit({
      ...
      reason: `mode=${payload.mode};source=${payload.source};segment=${payload.segment};q=${payload.search || '-'};plan=${payload.filters?.planState || 'all'};credits=${payload.filters?.creditsState || 'all'};channel=${payload.filters?.channelState || 'all'};activity=${payload.filters?.activityWindow || 'all'};payments=${payload.filters?.paymentsState || 'all'};sort=${payload.filters?.sortBy || 'created_desc'};cohort=${payload.filters?.cohortView || 'all'};rows=${payload.rowsCount};truncated=${payload.truncated ? 1 : 0};ids=${payload.userIds.length}`,
      oldJson: null,
      newJson: {
        mode: payload.mode,
