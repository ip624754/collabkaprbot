...
- Перед деплоем можно быстро прогнать `npm run smoke:broadcast-local-db-fuse`, чтобы поймать регресс local fuse в bounded implementation `src/jobs/broadcastDelivery/` (arming only on Redis-fuse write failure, precheck before Redis/DB touch).
