**STEP495:** OG preview refresh and repo cleanup — replaced the previous multi-file OG pack under `assets/social/` with the newly approved polished preview visual under stable names `assets/social/collabka-og.png` and `assets/social/collabka-og.webp`, removed legacy `collabka-og-1200x630*` and `*-alt*` assets, and aligned `index.html` plus `scripts/smoke-landing-contract.js` to the actual shipped image contract. To avoid deceptive size-encoded naming after the approved no-recrop visual decision, meta now points to the neutral primary asset name and declares the real source dimensions (`1730x908`). Scope stays landing/share-preview only: no bot runtime, callbacks, DB, monetization, accept/reply/unlock, or workflow-path logic changed.
