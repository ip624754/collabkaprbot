**STEP533:** Users interaction clarity / duplicate-state compression — finished the first real clarity-over-feature-growth pass on `/admin/users`. The sort and cohort rails now behave like unambiguous one-click controls: priority chips apply immediately and stay visibly active, while cohort counters are no longer faux-buttons and the actual cohort choice lives only in the chips below. In parallel, compressed repeated active-state badges across the screen (`Порядок`, local `Когорта`, local `Пресет`, local `Срез`, local `Закреплено`) so the top sticky state strip remains the single source of truth and section bodies stop re-explaining the same state three times. The table meta strip was also de-internalized (`Пользователи · рабочий список` instead of step noise), presets/compare/follow-up copy was shortened and russified, and the overall `/admin/users` screen now reads more like one disciplined control plane and less like stacked feature blocks. Scope stayed narrow and UI-only: no SQL changes, no new write paths, no route-family expansion.
