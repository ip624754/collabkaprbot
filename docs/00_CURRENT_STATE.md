**STEP437:** Brand application accept SQL typed params hotfix — fixed the live SQL failure behind `✅ Принять` where PostgreSQL could not infer parameter types inside `jsonb_build_object(...)` (`could not determine data type of parameter $2`). `src/db/queries.js` now casts `accepted_by_user_id` and `charged_cost` explicitly inside the accept transaction metadata write (`$2::bigint`, `$3::int`), so the sync-first accept path can actually commit the charge + `status=in_progress` transition instead of rolling back before COMMIT. The legacy helper `markBrandApplicationAccepted()` is hardened with the same explicit cast to avoid the same class of failure if it is reused later. Added `scripts/smoke-brand-app-accept-sql-contract.js` and wired it into preflight to freeze this typed-params contract.
