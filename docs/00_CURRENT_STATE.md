...
> Примечание: `CONTACT_UNLOCK_COST`, `CONTACT_UNLOCK_TTL_DAYS`, `BRAND_CREDITS_CACHE_TTL_SEC` читаются напрямую в `src/bot/bot.js` (не через `CFG`).
...
- В карточке заявки кнопка **✅ Принять** — это точка монетизации: открывает диалог и **списывает кредиты Brand Pass** (env: `BRAND_APP_ACCEPT_COST`, по умолчанию 1).
- UX: рядом с ✅ Принять показываем **кэшированный баланс кредитов (Redis-only)** и “хватит/не хватает”, чтобы не было сюрпризов.
- До принятия (status=new) **нельзя** ответить/отправить шаблон (иными словами — нельзя “обойти” списание). После принятия доступны «✍️ Ответить» и «⚡ Шаблоны».
