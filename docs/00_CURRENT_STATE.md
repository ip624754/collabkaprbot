**STEP497:** OG meta/share contract restore — fixed the public landing head meta to point back to the actual shipping canonical OG asset pair `assets/social/collabka-og-1200x630.png` and `.webp`, restoring the Open Graph/Twitter preview contract after a filename drift that left `index.html` referencing a non-existent `collabka-og.png`. Also restored the correct `1200x630` dimensions in meta so social scrapers and source-level smoke align with the real asset. Scope stays landing/share-meta only: no bot runtime, callbacks, DB, monetization, accept/reply/unlock, or workflow-path logic changed.
