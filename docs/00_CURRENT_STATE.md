...
- STEP258: «Поделиться витриной» — убран “сырой” URL из текста шаринга: вместо строки `🔗 Витрина: ...` теперь html-якорь `🔗 Открыть витрину` (кликабельно и без URL в превью). Для системного шаринга ссылку отдаём через параметр `url=` (t.me/share/url).
