...
- В тикете есть кнопка <b>✍️ Ответить</b> (ручной ответ) и быстрые шаблоны (1 клик).
- Шаблоны: ✅ Принято / ❓ Нужны детали / ✅ Сделали / ⏳ В работе.
- Ручной ответ: бот пришлёт подсказку. Просто ответь (reply) на сообщение бота одним текстом — бот доставит пользователю.
