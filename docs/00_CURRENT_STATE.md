**STEP459:** Brand quick-reply preview dedupe cleanup — tightened the brand-side quick-reply confirm step after `✅ Принял — дальше` so the second screen now behaves as a real preview/confirm screen instead of a duplicate template picker. The brand application preview keeps the same outgoing message content and send semantics, but drops the duplicate template-switch row, shows an explicit `Шаблон: …` line for the currently selected quick reply, and keeps only `📨 Отправить`, `🔁 Выбрать другой`, `✍️ Ответить`, plus the normal footer. No accept/charge/deal mutations were touched, no callback payloads were renamed, and no new DB reads were added in hot UI paths. Added `scripts/smoke-brand-app-preview-dedupe-contract.js`, wired into `package.json` and `scripts/preflight.js`.

**STEP458:** Brand application accept SQL typing hardening — fixed the remaining PostgreSQL parameter-typing tail in the brand-application accept family so `✅ Принять` no longer falls through into `db_error / could_not_determine_data_type_of_parameter_2` on live accept clicks. The accept SQL helpers now cast actor/cost values explicitly inside the JSONB mutation path, brand-side accept catch logging now records more useful runtime diagnostics, and preflight now includes the dedicated accept SQL contract smoke so this class of regression cannot slip through as a hidden source-only tail. Scope stayed narrow: accept/charge semantics are unchanged, credits still charge exactly once, and no new DB reads were added.
