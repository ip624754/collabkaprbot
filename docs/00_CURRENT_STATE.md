**STEP479:** Ops one-screen / circuit breakers refresh — promoted `docs/ops/02_HEALTH_ONE_SCREEN.md` from a generic top-down health cheat-sheet into the canonical operator one-screen matrix: `trigger → symptom → /api/health field → safe action → rollback/toggle`. The refreshed guide now covers the current live watchlist only: Redis degraded, broadcast pending snapshot staleness, broadcast DB overload / cooldown, payments runtime fallback, payments HMAC / payload issues, QStash reschedule failures, official publish stuck, and hard-skip spikes. No runtime, DB, callback, or payment semantics changed. This is an operator/docs step so the on-call path is faster and safer: look at health/Admin → Ops, make one reversible move, re-check health, and only then go to deeper runbooks/logs. Owner/readiness/handoff docs were synced to point at this one-screen layer first.

**STEP478:** Runtime-grade Stars payment validation integration — added `scripts/test-runtime-stars-payment-validation.js`, a real runtime-grade integration test for the strict Telegram Stars invoice validation path. Instead of another source-only grep, the test dynamically imports the live `src/bot/bot.js` module through Node `registerHooks()` with minimal stubs for external npm packages (`grammy`, `pg`, `@upstash/redis`, `pino`, `dotenv/config`), so it remains runnable even on bare source snapshots without `node_modules`. Covered invoice families: `pro`, `brand_pass` (canonical + legacy numeric credits token), `brand_plan` alias compatibility, `matching`, `featured`, `official_publish`, `founder`, plus negative cases for `bad_currency`, `amount_mismatch`, `payer_mismatch`, and `bad_payload_chars`. Wired into `package.json` and `scripts/preflight.js`, so `preflight:source` now catches a class of runtime/module-load/business-logic regressions in the strict money entry path that string-contract smokes can miss. Scope is test/preflight/docs only: no payment semantics, no DB schema, and no callback/runtime UX changed.

**STEP477:** IG parked hard gate — added `scripts/smoke-ig-parked-hard-gate.js` and wired it into `package.json` + `scripts/preflight.js` so the parked Instagram OAuth subtree cannot silently re-enter the active deploy/runtime surface. The new guard fails fast if `api/ig/oauth/*` comes back under active `api/`, if `src/lib/igOAuth.js` or `src/lib/cryptoBox.js` return to the active runtime tree, if `vercel.json` rewrites expose parked IG routes, or if active source starts importing `_ig_oauth_parked/*` directly. Also strengthened `_ig_oauth_parked/README.md` so future audits see the hard rule in-tree. Scope is source/deploy-surface safety only: no bot runtime, DB, callback, or money-path behavior changed.

**STEP476:** Broadcast stale visibility + tiny 429 atomicity hardening — surfaced pending broadcast snapshot staleness to operators without changing broadcast routing semantics: `src/bot/cron.js` now stamps `stale_after_sec` into the Redis pending snapshot, `/api/health` exposes pending snapshot age/stale state, and Admin → Ops / Clear pending snapshot now show age + stale warnings. In parallel, the rolling broadcast `429users` set moved from raw `SADD + EXPIRE` into atomic Redis Lua via `saddCardWithExpire(...)` in `src/lib/redis.js`, so cooldown-window cardinality no longer depends on two separate commands. No DB schema changes, no audience/routing changes, no money-path changes, and no new hot-path DB reads.

**STEP475:** Payments fallback guardrails — hardened the existing runtime fallback operator toggle without changing payment apply semantics. `src/lib/paymentsOps.js` now clamps runtime fallback windows to the intended bounded range (5 min .. 24 h), exposes derived runtime age/active-hours data, and adds a Redis-only guardrail heartbeat: while runtime fallback stays ON, ops digest gets a reminder approximately every 2 hours; if a stale zombie runtime state somehow survives past expiry / hard-cap, the guardrail auto-disables it and emits an ops event. `src/bot/cron.js` now runs this best-effort guardrail sweep from the regular tick path, `api/health.js` exposes `payments.fallback_apply_age_sec` + `payments.fallback_apply_hours_active` and richer runtime guard metadata, `src/bot/adminOpsText.js` shows active-hours/reminder context in Admin → Ops, and `src/bot/bot.js` shows bounded-TTL / reminder copy inside `🧯 Payments fallback apply` plus active-hours in `⚙️ Админка → Система`. Updated source smokes: `scripts/smoke-admin-payments-fallback-contract.js`, `scripts/smoke-admin-ops-render.js`, and `scripts/smoke-health-admin-shape.js`. No DB schema changes, no payment validation changes, no new hot-path DB reads.
