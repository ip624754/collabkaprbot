**STEP528:** Users rails hierarchy compression — compressed the upper `/admin/users` control plane into two clearer priority levels so the page breathes better on the real operator window without adding any new logic. Level 1 now holds the true slice-building controls only (`sort / priority`, `cohort`, `filter rail`) inside a compact 3-card grid, while Level 2 collects the lighter helper layer (`saved presets`, `URL-backed working view`, `pagination`) in a smaller secondary cluster. The old verbose preset-card block was flattened into preset pills, the upper notes were trimmed, and follow-up cards were tightened, so the rails stop feeling like stacked mini-pages. Scope stays read-only and reversible: no server-contract expansion, no new write paths, no DB changes, no new route family — only hierarchy compression and density polish on top of STEP527.
