**STEP575:** Invite Center user UX reframe — tightened the main user-side invite hub around the already-landed STEP571–574 reward truth without changing reward math, ledger semantics, redeem economics, or admin visibility. `src/bot/bot.js` now removes the old raw-link + 3 ready-text dump from the primary surface and rebuilds it as a shorter action-first Invite Center: `Summary` (`Invited / Activated / Activation rate / Activation rule`), `Actions` (`Share invite / Link + copy / Invite card`), `Points`, `Reward`, and `Recent invited contacts` (up to 3 rows with a history hint when more exist). The history/redeem surfaces stay separate, but wording is aligned to the cleaner contract (`Invite history`, `Redeem`, `Link + copy`, `Invite card`) and the link screen now holds the raw link + invite code instead of the main hub. A new source smoke `scripts/smoke-invite-hub-ux-contract.js` guards the new hub contract. Scope stays intentionally narrow: no data-model changes, no new reward logic, no callback family expansion, and no admin redesign. Live verification still required for Telegram readability, conditional buttons, recent-contact density, and the updated link/card wording.
