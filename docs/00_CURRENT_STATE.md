**STEP535V:** Env/docs baseline sync + schema guard — synced `.env.example`, `docs/91_PROD_LAUNCH_30MIN.md`, `docs/92_PROD_ENV_BASELINE.md`, and both QStash runbooks with the real env/config surface (`QSTASH_URL` now documented as optional/parity alongside token/signing keys), added a dedicated source smoke to guard contact unlock against reading `profile_*` directly from `workspaces`, and patched `src/db/queries.js` so `unlockWorkspaceContactsWithCredits()` reads profile/contact fields from `workspace_settings` via `left join` instead of the legacy table shape.
