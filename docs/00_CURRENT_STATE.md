**STEP541:** Callback branch feedback normalization — normalized the remaining branch-local double-ack collisions that were still undermining Telegram callback honesty after STEP536. Scope stayed intentionally narrow and bot-only: in `src/bot/bot.js`, the notification dismiss branch (`a:nd`) no longer sends a bare callback ack before the first-tap confirmation toast, and brand application delete (`a:brand_app_del_do`) no longer sends an unconditional empty ack ahead of its deny/success feedback. In `src/bot/gwAccess.js`, the no-access branch inside `renderGwAccess(...)` now sends exactly one final no-access callback response instead of a bare ack followed by a second text ack. No payload contracts, guards, degraded policies, money/publish paths, or surrounding routing structure changed. Source smoke is green (`callbacks:check`, `actions:check`, `preflight:source`, `node --check` on touched bot files); live Telegram verification is still required for `a:nd` double-tap UX, brand-app delete feedback, and giveaway-access no-access feedback.
