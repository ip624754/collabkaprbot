...
**STEP432:** Future spec for Creator → Brand super-admin OPS COPY clarify + ENV on/off — formalized the recommended minimal improvement for super-admin copies of creator→brand applications as a future-only spec card. This is **not** active runtime work: current behavior still allows creator applications to fan out to owner + managers + `SUPER_ADMIN_TG_IDS`. The future candidate improvement is to keep brand-side delivery unchanged, relabel super-admin notifications as `🛠 OPS COPY`, and gate those copies behind a single env flag `BRAND_APP_SUPERADMIN_COPY_ENABLED=1|0`, without adding admin inbox screens, new runtime state, or hot-path DB reads.
...
**STEP432 spec card (future-only / watchlist):**
- **Problem to clarify:** creator → brand applications currently fan out not only to brand owner/managers but also to `SUPER_ADMIN_TG_IDS`, which is useful as operator oversight yet easy to misread as “my own admin inbox”. The future step is about clarifying that meaning, not creating a second workflow.
- **Goal:** keep the existing brand-side application pipeline unchanged while making super-admin visibility explicit and optionally disable-able.
- **Safe change shape:** add exactly one env/config switch `BRAND_APP_SUPERADMIN_COPY_ENABLED=1|0` and relabel only the super-admin copy as `🛠 OPS COPY · Заявка креатора бренду` with a short line that the main workflow stays with the brand. Brand owner/manager text and action flow must stay unchanged.
- **Default / rollout stance:** default should remain `1` for zero-regression rollout; the new env only makes the current copy behavior explicit and reversible. Switching to `0` later is an operator choice, not a logic migration.
- **What must NOT happen:** no new admin inbox, no read-only/break-glass subsystem yet, no new Redis/DB toggles, no callback/state-machine split, no extra DB reads in menu/home/hub paths.
- **Before runtime rollout:** add a narrow contract smoke that freezes (a) owner/manager recipients stay intact, (b) super-admin copies honor the env flag, and (c) only the super-admin variant gets the `🛠 OPS COPY` relabel.
