**STEP494:** Release-consistency restore — restored the missing root `.env.example` so release/source preflight can start from a bare FULL baseline again, rebuilt `package-lock.json` into a real resolved lockfile that matches the current dependency graph, and tightened the static lock guard so `scripts/check-package-lock.js` now fails if direct dependencies are present only in the root dependency map but missing resolved `lock.packages` entries. `scripts/preflight.js` source-only mode now runs this lockfile gate immediately after the ENV baseline contract, so a truncated or drifted release snapshot fails fast before deeper smoke gives a misleading “clean” signal. Scope stays release/docs/tooling only: no bot runtime, callbacks, DB schema, monetization, accept/reply/unlock, or hot-path query behavior changed.
