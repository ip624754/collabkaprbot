**STEP501:** Web Admin User Card usable v1 — upgraded `/admin/users/[id]` from a thin technical drilldown into a real operator card without widening the risk surface. The card now renders one aggregated `section=user` snapshot with five clear blocks: summary header, `Профиль`, `Доступ и сигналы`, `Активность`, `Заметка оператора`, plus a light `Последние admin-действия` list filtered to the selected user. `src/lib/adminWeb/readModels.js` now returns explicit `account`, `access`, `activity`, `note`, and `recentAdminAudit` sections in one response, while `api/admin-web-write.js` hardens note writes by normalizing blank input to clear, tagging audit rows as `set_user_note` / `clear_user_note`, and storing old/new note snapshots. UI remains hobby-safe: one primary read request on page load, no polling, no cron dependency, and no dangerous mutations (payments, segments, channels, deals) added. Collateral consistency fix: the stale legacy `api/admin-web/*` split files were removed from the uploaded baseline so the repo once again matches the STEP500A collapsed-handler contract guarded by source smoke.
