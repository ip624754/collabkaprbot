**STEP503:** Web Admin User Card polish + operator usability — strengthened `/admin/users/[id]` into a real operator screen without widening the risk surface. The user card now preserves list context (`q` / `segment`) in URL-driven back navigation, shows a clearer state hint + denser summary header, upgrades the `Аккаунт и доступ` and `Активность` blocks from raw details to operator summaries, and polishes note UX with inline status, blank→clear normalization, timestamp/actor metadata, and short recent admin trace. `userDetail` stays hobby-safe as one aggregated read model, while note writes now record `set_user_note` / `clear_user_note` audit rows with old/new snapshots. As a consistency fix, stale split `api/admin-web/*` files were removed again so the repo matches the collapsed Hobby-safe function-budget contract.
