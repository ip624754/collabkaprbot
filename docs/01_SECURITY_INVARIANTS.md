...
- Приоритет после unlock: **structured → (если пусто) legacy** (`profile_contact`, `profile_ig`, `profile_portfolio_urls`).
- Перенос legacy → structured — только по явной кнопке и только при **однозначном распознавании** (один тип, одно значение).
  - если в legacy несколько типов/вариантов — отказываемся и просим заполнить вручную;
