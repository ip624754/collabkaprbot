...
- Сущность считается сделкой только при наличии `meta.deal.accepted_by_user_id` **и** `meta.deal_stage`; прямой callback не может создать/изменить этап до принятия.
- Все deal-only view/reply/template/stage пути повторно проверяют actor access и accepted-deal evidence; SQL writer содержит тот же accepted-only guard.
