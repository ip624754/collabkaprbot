...
### STEP258 — «Поделиться витриной»: без “сырого” URL, ссылка спрятана в «Открыть витрину»
Задача:
- полностью убрать видимый URL из текста шаринга, чтобы получатель не видел “вот ссылка” ни в превью, ни в самом тексте.

Изменения:
- в `buildWsShareText()` строка `🔗 Витрина: <url>` заменена на html-якорь: `🔗 Открыть витрину` (кликабельно, без вывода URL).
- системный шаринг через `t.me/share/url` теперь отдаёт ссылку параметром `url=` (текст остаётся чистым, без URL).
- в меню `🔗 Поделиться витриной` вместо вывода полного URL показывается ссылка `Открыть витрину`.

Как проверить (smoke):
- Creator → `🔗 Поделиться витриной` → выбрать `📄 Коротко`.
- В тексте нет строки `🔗 Витрина: https://...`; вместо неё — `🔗 Открыть витрину` (кликается).
- Нажать `📨 Отправить` → откроется системный шаринг; текст без URL, ссылка передаётся через `url=`.
- Повторить для `📄 Подробно`.

Риск регрессий: **низкий** (только форматирование текста + параметр share/url; бизнес‑логика не затрагивается).
