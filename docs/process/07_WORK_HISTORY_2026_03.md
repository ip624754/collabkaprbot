...
### STEP291 — Admin Broadcast: audit + UX hardening + docs alignment
Контекст:
- При аудите рассылок нашли 2 UX‑риска в админском broadcast‑флоу:
  1) на первом экране «Новая рассылка» кнопка «Отмена» вела в `a:admin_home` и **не очищала** `expectText=bc_content` → админ мог “отменить”, но остаться в режиме ожидания контента;
  2) на ряде экранов рассылки (кнопки/аудитория/превью/список/карточка) не было стандартного admin‑футера `📋 Меню / 🏠 Home`, что повышало риск тупиков.

Цель:
- Сделать broadcast‑контур в админке предсказуемым: отмена всегда снимает режим ввода и чистит черновик, а на ключевых экранах есть единая навигация.
- Привести документацию broadcast к фактической поддержке контента (в коде есть видео/GIF/документ, poll не поддерживается).

Изменения (код + docs):
- `src/bot/bot.js`:
  - `a:bc_start`: «❌ Отмена» теперь ведёт в `a:bc_cancel` (очищает expectText/черновик) + добавлен `kbAdminFooter`.
  - `bc_content` (expectText handler): на неподдерживаемом формате «Отмена» ведёт в `a:bc_cancel`; на квитанции «Контент сохранён» добавлен `kbAdminFooter`.
  - Экраны broadcast (кнопки/шаблоны кнопок/аудитория/превью/список/карточка): добавлен `kbAdminFooter` для единых `⬅️ ... / 📋 Меню / 🏠 Home`.
- `docs/00_CURRENT_STATE.md`: broadcast‑раздел уточнён — доступ только суперадмину; контент: text/photo/video/GIF/document (poll убран).
- `docs/audit/21_BROADCAST_E2E_SMOKE_2026_03.md`: сценарий A заменён на Access control (не‑админ не может открыть рассылки); добавлен сценарий управления (список/карточка/пауза/стоп).
- `smoke-tests_short.md`: секция 14 обновлена (admin‑access, управление, корректная навигация).

QA:
- Админ: `👑 Админка → 🧰 Операции → 📣 Рассылка`:
  - старт → «❌ Отмена» снимает режим ввода (после отмены обычный текст не должен трактоваться как bc_content);
  - пройти: контент → кнопки → аудитория → превью → подтверждение;
  - везде есть `📋 Меню / 🏠 Home`.
- Не‑админ: клики по старым кнопкам `a:bc_*` дают `Нет доступа.`.

Риск регрессий: **низкий** (изменения локальны в админском контуре и навигации; публичные флоу не затронуты).
