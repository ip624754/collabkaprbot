...
23) **Broadcast deliver → local DB overload fuse contract smoke**  
   Source-level smoke `scripts/smoke-broadcast-local-db-fuse.js` проверяет bounded implementation через `scripts/lib/broadcast-delivery-source-reader.js`: local fuse находится в `src/jobs/broadcastDelivery/dbOverload.js`, а precheck orchestration — в `src/jobs/broadcastDelivery/delivery.js` для редкого сценария `DB overloaded + Redis unavailable`: module-level state `localDbDegradedUntilMs`, helpers `getLocalDbOverloadFuseTtlMs/getLocalDbOverloadFuseUntilMs/armLocalDbOverloadFuse`, arming local fuse только при провале записи Redis-fuse, precheck `local_fuse_precheck` **до** Redis fuse read и **до** `db.getBroadcast(...)`, а `respondDbOverloadFuse(...)` маркирует путь флагом `local_fuse`. Это ловит регресс, при котором warm instance продолжает жечь Neon, хотя Redis уже недоступен и локальный short-circuit должен сработать.
...
- На `QStash dedup sanitize contract` → проверь `src/lib/qstash.js`: raw readable dedup inputs (`qping:*`, `mon:autoheal:*`, broadcast/official publish ids) должны оставаться на call-sites, но `qstashPublishJSON()` обязан централизованно санитизировать `Upstash-Deduplication-Id` до safe charset без `:` и держать admin ping helper-text неманипулятивным (не сваливать enqueue failures только на signing keys).
...
- На `Brand application super-admin OPS COPY contract` → проверь creator→brand notify fanout: owner/manager должны оставаться на исходном `notifText`, super-admin copies должны идти только при `BRAND_APP_SUPERADMIN_COPY_ENABLED=1` и только в relabeled варианте `🛠 OPS COPY`, без нового admin inbox и без дрейфа `.env.example` / `docs/92_PROD_ENV_BASELINE.md`.
- На `Brand application accept UX contract smoke` → проверь `renderBrandAppView()`, `acceptBrandApplication()`, worker `api/qstash/monetization-retry.js` и brand-app reply fallback: pending/success CTA после `✅ Принять` не должны уводить в общий `📥 Inbox`, copy обязана честно говорить про async списание кредита, follow-up routing должен вести в `📨 Открыть заявку / 💬 В работе`, а Redis-only credits block должен оставаться видимым на карточке после accept.
