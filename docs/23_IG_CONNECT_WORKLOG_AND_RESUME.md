...
> TL;DR: Comment‑верификацию (Level B) решили не использовать из‑за утечек username. Перешли на OAuth‑only (Level A), но Meta начала возвращать `pages=0` и местами блокировать доступ. Поэтому **UI скрыли**, код оставили.
...
- код/эндпоинты не удаляем, чтобы быстро вернуться позже.
