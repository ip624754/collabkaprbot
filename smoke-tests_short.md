...
## 15) Admin (E2E: Users/Payments/Outbox/System/Broadcast)
Коротко:
- Users: поиск, карточка, заметка, отправка сообщения. Важно: `📋 Меню`/`🏠 Home` снимают input‑mode (следующий текст не улетает в поиск/заметку).
- Payments: список → просмотр → (если есть кейс) ручной apply без дублей.
- Outbox: в группе сниппеты скрыты (🔒), в DM — видны; повтор/предпросмотр работает.
- System: QStash статус открывается, signed ping отвечает.
- Broadcast: старт → `❌ Отмена` снимает input‑mode; создать тестовую рассылку текстом.

Полный сценарий (10–12 минут): `docs/audit/25_ADMIN_E2E_SMOKE_2026_03.md`.
