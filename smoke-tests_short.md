...
## 14) Broadcast (E2E: gate + create + cooldown)
Коротко:
- **Gate:** если нет подключённого канала, `➕ Новая рассылка` показывает понятный экран «Подключить канал / Мои каналы / Назад / Меню / Home» (нет тихого возврата).
- **Создание:** ввести текст, добавить до трёх URL‑кнопок (вручную или shortcuts), отправить. Финальный экран «✅ Рассылка #… завершена» содержит кнопки «Открыть» / «📋 Меню» / «🏠 Home».
