...
assert.ok(
  botSrc.includes(".text(brandAppListReturnButtonLabel(), `a:brand_apps|ws:0|s:${back.status}|p:${back.page}`)") &&
    ...
    botSrc.includes("kb.text(creatorBrandAppListReturnButtonLabel(), 'a:my_apps|p:0').text('📋 Меню', 'a:menu').text('🏠 Home', 'a:home');") &&
    botSrc.includes("kb.row().text(creatorLeadListButtonLabel(), listCb).text('📋 Меню', 'a:menu').text('🏠 Home', 'a:home');"),
