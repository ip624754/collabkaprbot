...
assert.ok(bot.includes("from './domains/adminAuth/index.js'"), 'bot must import the bounded admin-auth facade');
...
assert.ok(bot.includes('admin_web_auth: (ctx2, p2) => handleAdminAuthChallengeCallback(ctx2, p2)'), 'bot must wire the exact admin auth owner to the domain handler');
...
assert.ok(callbackRoute.includes("parseAdminAuthDecisionPayload(payload)"), 'auth callback module must own the action match');
...
assert.ok(callbackActions.includes("DECIDE: 'a:aw_auth_dec'"), 'admin-auth domain must own the exact callback action');
assert.ok(callbackDomainRoute.includes('phase: CALLBACK_PHASE.PRE_USER'), 'admin auth challenge ownership must be pre-user');
assert.ok(callbackDomainRoute.includes('phase: CALLBACK_PHASE.POST_USER'), 'admin auth control ownership must be post-user');
